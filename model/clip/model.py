...
class AttentionPool2d(nn.Module):
    """
    The attention pooling is implemented as a single layer of “transformer-style” multi-head QKV attention where the
    query is conditioned on the global average-pooled representation of the image.
    """
...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        # Learned proj of image to embed i.e. linear projection to shared image-text space
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))

    def forward(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid]. This creates image patches!!
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # Learned proj of text to embed i.e. linear projection to shared image-text space
