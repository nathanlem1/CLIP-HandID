...
def freeze_text_encoder(model):
    """
    This function freezes the text_encoder of the CLIP model.
    Note that:
     param.requires_grad = False: Freezes the parameter. No gradient will be calculated for it, and it won’t be updated.
     param.requires_grad = True: This is the default. Gradients will be calculated, and the parameter will be updated
...
def main():
    ...
    if args.backbone_name == 'RN50':
        args.batch_size = 4  # For better performance: batch_size=4 for CLIP RN50 & batch_size=20 for ViT-B/16.
    ...
    transform_train_list = [
        ...
        transforms.RandomCrop((args.input_size[0], args.input_size[1])),  # input_size = (224,224)
    ...
    if hasattr(model, 'image_encoder'):  # We fine-tune image_encoder only i.e. text_encoder in NOT fine-tined.
        base_param_ids = set(map(id, model.image_encoder.parameters()))
        new_params = [p for p in model.parameters() if
                      id(p) not in base_param_ids]
        param_groups = [
            {'params': filter(lambda p: p.requires_grad, model.image_encoder.parameters()), 'lr': args.lr},
            {'params': filter(lambda p: p.requires_grad, new_params), 'lr': 10*args.lr}]
    else:
    ...
    if args.optimizer == 'sgd':
        ...
        raise ValueError('Set the optimizer to either sgd, adam or adamW')
